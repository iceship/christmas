openerp.point_of_sale = function(db) {
    ...
    var Pos = Backbone.Model.extend({
        initialize: function(session, attributes) {
            ...
            $.when(this.get_app_data(),this.fetch('pos.category', ['name', 'parent_id', 'child_id']),
                this.fetch('product.product', ['name', 'list_price', 'pos_categ_id', 'taxes_id', 'product_image_small', 'ean13', 'id','price'], [['pos_categ_id', '!=', false],['price','=',this.get("pricelist_id")]]),
        ...
        get_app_data: function() {
            var self = this;
            return $.when(new db.web.Model("res.users").get_func("read")(this.session.uid,['shop_id']).pipe(function(result) {
            	self.set({'shop_id':result['shop_id'][0]});
            	return new db.web.Model("sale.shop").get_func("search_read")([]).pipe(function(result) {
                	...
                	return new db.web.Model("res.company").get_func("read")(company_id, ['currency_id', 'name', 'phone', 'logo', 'rml_header1', 'street', 'city', 'zip', 'state_id', 'country_id']).pipe(function(result) {
    ...
    var Orderline = Backbone.Model.extend({
        ...
        initialize: function(attributes) {
            ...
            this.bind('change:quantity', function(unused, qty) {
                if (qty == 0)
                    this.trigger('killme');
                return $.when(new db.web.Model("pos.order").get_func("get_price")(this.get("id"),this.get("quantity")).pipe(_.bind(function(result) {
            		this.set({
            			price : result,
            		});
            	},this)));
    ...
    var Order = Backbone.Model.extend({
        ...
        initialize: function(attributes){
            ...
            $('input#with-invoice').change(_.bind(this.checkboxChange, this));
            return this;
        },
        events: {
        ...
        getDiscountPercentage:function(){
			return ($('input#discount-amount').val()/this.getTotal()*100).toFixed(4);
        },
        getDiscountAmount:function(){
			return ($('input#discount-percentage').val()*this.getTotal()/100).toFixed(2);
        },
        checkboxChange: function(){
            if (($('input#with-invoice').attr('checked')) === 'checked'){
                netAmount = this.getNetAmount()
                taxAmount = this.getTax()
                $('#tax-amount').html(taxAmount.toFixed(2));
                $('#net-total').html(netAmount.toFixed(2));
            }
            else{
                netAmount = this.getNetAmount()
        ...
        getNetAmount: function(){
            netAmount = this.getTotalTaxExcluded()-parseFloat($('input#discount-amount').val());
            if (($('input#with-invoice').attr('checked')) === 'checked'){
                taxAmount = this.getTax()
    ...
    var PaymentWidget = db.web.OldWidget.extend({
        ...
        validateCurrentOrder: function() {
            ...
            currentOrder = this.shop.get('selectedOrder');
            pos.pushOrder(currentOrder.exportAsJSON()).pipe(_.bind(function() {
            	disc = pos.get('disc')
            	if (disc != false){
            	...
            	if(pos.get('company_state') != false )
            		$("#company_state").html(", " + pos.get('company_state'));
            	else
					$("#company_state").html("");
            	if(pos.get('company_country') != false )
            		$("#company_country").html(", " + pos.get('company_country')+"<br/>");
            	else
...
//        update_payment_line: function(cashRegister) {
//        	var newPaymentline, amount;
//        	this.currentPaymentLines.each(_.bind( function(newPaymentline) {
...
////                remaining = remainingAmount > 0 ? 0 : (-remainingAmount).toFixed(2);
//        		new_amount = 0.00
//        		org_amount = parseInt(newPaymentline.get('amount'));
////        		new_amount = remaining
//        		newPaymentline.set({
//                    'amount':org_amount+1
//                });
////    		return (this.get('paymentLines')).update(newPaymentline);
//            }, this));
//        },
        ...
        updatePaymentSummary: function(cashRegister) {
            ...
            //if (this.$element.find('#with-invoice').checked){
//                this.$element.find('#tax-amount').html(taxAmount.toFixed(2));
//            }
//            else{
//                this.$element.find('#tax-amount').html('0.00');
//            }
            remainingAmount = netTotal - paidTotal;
            remaining = remainingAmount > 0 ? 0 : (-remainingAmount).toFixed(2);
//            this.set({'rem_amt' : remaining})
            $('#payment-remaining').html(remaining);
            $('#payment-remaining_last').html(remaining);
//            this.update_payment_line();
        },
        setNumpadState: function(numpadState) {
        	if (this.numpadState) {
        		this.numpadState.unbind('setValue', this.setValue);
    ...
    var ReceiptWidget = db.web.OldWidget.extend({
        ...
        init: function(parent, options) {
            ...
            this.company_state = pos.get('company_state');
    		this.company_country = pos.get('company_country');
            this.shop_obj = pos.get('shop');
            this.amount_in_words = pos.get('amount_in_words');
        ...
        calculateNetTotal:function(){
			alert(JSON.stringify(this.shop.get('selectedOrder')));
        },
    ...
    var App = (function() {
        ...
        App.prototype.category = function(id) {
            var c, products,salesmens,partners, self = this;
            ...
            var getProductByEAN = function(ean) {
                ...
                if (prefix in {'02':'', '22':'', '24':'', '26':'', '28':''}) {
                    ...
                    if (scannedPackaging !== undefined) {
                        ...
                        scannedProductModel.price = Number(ean.substring(7,12))/100;
                    }
