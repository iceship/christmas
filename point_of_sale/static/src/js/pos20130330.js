    ...
    var Pos = Backbone.Model.extend({
        initialize: function(session, attributes) {
            ...
            $.when(this.get_app_data(),this.fetch('pos.category', ['name', 'parent_id', 'child_id']),
                this.fetch('product.product', ['name', 'code', 'list_price', 'pos_categ_id', 'taxes_id', 'product_image_small', 'ean13', 'id','price'], [['pos_categ_id', '!=', false],['price','=',this.get("pricelist_id")]]),
		...
		var m, s,myproduct;
                ...
                if (s) {
                    m = products.filter( function(p) {
                        return (String(p.code).indexOf(s) != -1) || (p.name.toLowerCase().indexOf(s) != -1);
                    });
                    $('.search-clear').fadeIn();
                } else {
                    m = products;
