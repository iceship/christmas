...
class pos_order(osv.osv):
    ...
    def create_account_move(self, cr, uid, ids, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            user_id = uid
            if order and order.user_id and order.user_id.id:
                user_id = order.user_id.id
            comp_id = res_obj.browse(cr, user_id, user_id).company_id
            ...
            for line in order.lines:
                ...
                analytic_account_id = order.analytic_account_id and order.analytic_account_id.id or False
                vals= {
                       'name': line.name,
                ...
                move_line_id = account_move_line_obj.create(cr, uid, vals, context=context)
                uos_id = line.product_id.uos_id and line.product_id.uos_id.id or False
                ##ADDED TO CREATE ANALYTIC ENTRY ON MOVE CREATION FROM FROM POS BACKEND BY ZESTY BEANZ##
                if analytic_account_id:
                    analytic_line = {
                                     ...
                                     'general_account_id': income_account,
                                     'journal_id': order.sale_journal.analytic_journal_id and order.sale_journal.analytic_journal_id.id,
                                     'ref': line.name,
                                     'move_id': move_line_id
                                     }
                    self.pool.get('account.analytic.line').create(cr, uid, analytic_line, context=context)
                    ##END##
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        return True
    def get_price(self, cr, uid, product_id, id, context={}):
        product_list_price = self.pool.get('product.product').browse(
            cr, uid, product_id, context=context)
        return product_list_price.list_price
pos_order()
...
class res_users(osv.osv):
    ...
    _columns = {
        ...
        'shop_id': fields.many2one('sale.shop', 'Users'),
    }
res_users()
