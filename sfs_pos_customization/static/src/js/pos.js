openerp.sfs_pos_customization = function(db) {
    ...
    var Pos = Backbone.Model.extend({
        initialize: function(session, attributes) {
            ...
            $.when(this.fetch('pos.category', ['name', 'parent_id', 'child_id']),
                this.fetch('product.product', ['name', 'qty_available', 'code', 'list_price', 'pos_categ_id', 'taxes_id', 'product_image_small', 'ean13', 'id', 'default_code', 'price'], [['pos_categ_id', '!=', false], ['price','=',this.get("pricelist_id")]]),
                this.fetch('product.packaging', ['product_id', 'ean']),
                this.fetch('account.bank.statement', ['account_id', 'currency', 'journal_id', 'state', 'name', 'curr_rate', 'comp_curr_rate'],
                    [['state', '=', 'open'], ['user_id', '=', this.session.uid]]),
                this.fetch('account.journal', ['auto_cash', 'check_dtls', 'currency', 'name', 'type', 'curr_rate', 'comp_curr_rate']),
                this.fetch('account.tax', ['amount', 'price_include', 'type']),
                this.fetch('res.partner', ['name', 'id'], [['customer', '=', true]]),
                this.fetch('res.users', ['name', 'id', 'shop_id']),
                this.get_app_data())
        ...
        get_app_data: function() {
            ...
            return $.when(new db.web.Model("sale.shop").get_func("search_read")([]).pipe(function(result) {
                ...
                var default_pos_partner = result[0]['default_partner_id']
                $.each(result,function(i,item){
                    if(item['id'] == self.get('shop_id')){
                            self.set({'shop': item});
                            company_id = item['company_id'][0];
                        }
                    });
                self.set({'default_pos_partner': default_pos_partner})
                self.set({'pricelist_id':self.get('shop')['pricelist_id'][0]});
                return new db.web.Model("res.company").get_func("read")(company_id, ['currency_id', 'name', 'phone', 'logo', 'rml_header1']).pipe(function(result) {
        ...
        _int_flush : function() {
            ...
            return new db.web.Model("pos.order").get_func("create_from_ui")([op]).fail(function(unused, event) {
                ...
                if (result instanceof Array && result.length === 2 && result[0] === "percent"){
                    alert("El descuento excede su limite permitido!!\nEl limite máximo para usted es de " + result[1] + "%");
                    this.set({'disc':false});
                ...
                return new db.web.Model("pos.order").get_func("read")(result,['amount_in_words','vat','name', 'company_state', 'company_country', 'partner_address', 'company_address', 'warehouse_address']).pipe(_.bind(function(result) {
                    this.set({'disc':true});
                    this.set({'amount_in_words':result[0]['amount_in_words']});
                    this.set({'vat':result[0]['vat']});
                    this.set({'name':result[0]['name']});
                    //this.set({'company_state':result[0]['company_state']});
                    //this.set({'company_country':result[0]['company_country']});
                    this.set({'partner_address': result[0]['partner_address']});
                    this.set({'company_address': result[0]['company_address']});
                    this.set({'warehouse_address': result[0]['warehouse_address']});
    ...
    var Salesmen = Backbone.Model.extend({
    });

    var SalesmenCollection = Backbone.Collection.extend({
        model: Salesmen,
    });
    ...
    var Orderline = Backbone.Model.extend({
        ...
        getAllPrices: function() {
            var self = this;
            var base = (this.get('quantity')) * (this.get('price')) * (1 - (this.get('discount')) / 100);
            var totalTax = base;
            var totalNoTax = base;
    ...
    var Paymentline = Backbone.Model.extend({
        ...
        exportAsJSON: function(){
            return {
                ...
                curr_rate: this.get('curr_rate'),
                comp_curr_rate: this.get('comp_curr_rate')
            };
        },
    });
    ...
    var Order = Backbone.Model.extend({
        ...
        initialize: function(attributes){
            var ele = document.getElementById('salesman');
            //$('.pos-payment-container').removeChild('.sfs_pos_mod')
            if (ele === null){
                $('.pos-payment-container').append(QWeb.render('sfs-pos-partner-template', {widget: self}))}
            Backbone.Model.prototype.initialize.apply(this, arguments);
            this.set({
        ...
        getNetAmount: function(){
            return this.getTotal()-parseFloat($('input#discount-amount').val());
        },
        validate_desc:function(){
            $('button#validate-order').attr('disabled', 'disabled');
            var total = parseFloat(0);
            payment_line_list = this.get('paymentLines');
            (this.get('paymentLines')).each(_.bind( function(item) {
                amount = item.get('amount')
                total += parseFloat(amount);
            }));
            paymentdue = parseFloat($("#net-total").text());
            if(total.toFixed(2) >= paymentdue){
                $('button#validate-order').removeAttr('disabled');
            }
        },
        setDiscountPercentage:function(){
            $('input#discount-percentage').val(this.getDiscountPercentage());
            $('#discountamount').html(this.getDiscountAmount());
            this.setNetTotal();
            this.validate_desc();
            this.setChangeAmount();
        },
        setDiscountAmount:function(){
            $('input#discount-amount').val(this.getDiscountAmount());
            $('#discountamount').html(this.getDiscountAmount());
            this.setNetTotal();
            this.validate_desc();
            this.setChangeAmount();
        ...
        setNetTotal: function(){
            $("#net-total").html((this.getNetAmount()).toFixed(2));
            $("#netamount").html((this.getNetAmount()).toFixed(2));
            var Namount1 = this.getNetAmount().toFixed(1);
            
            $("#amount_in_words").html('');
            var Namount = parseFloat(Namount1);
            var cmpid = parseFloat('3');
            var OrderName = this.get('name')
            $.when(new db.web.Model("sale.shop").get_func("search_read")([]).fail(function(unused, event) {
                event.preventDefault();
                $("#name").html(OrderName);
                pos.set({name:OrderName});
            }).pipe(_.bind(function(result1) {
                    this.company_id = result1[0]['company_id'][0];
            
             
            return $.when(new db.web.Model('pos.order').get_func('convert')([1],Namount,this.company_id).fail(function(unused, event) {
                event.preventDefault();
            }).pipe(_.bind(function(result) {
                $("#amount_in_words").html(result);
        ...
        setChangeAmount: function(){
            var change, paidTotal, remaining, dueTotal, currentOrder;
            paidTotal = this.getPaidTotal();
            dueTotal = this.getNetAmount();
            ...
            $('#payment-remaining_last').html(remaining);
        },
        getDiscountPercentage:function(){
            discount_percent = $('input#discount-amount').val()/this.getTotal()*100
            if (isNaN(discount_percent)){
            discount_percent = 0.00
            }
            return discount_percent.toFixed(4);
        },
        getDiscountAmount:function(){
            return ($('input#discount-percentage').val()*this.getTotal()/100).toFixed(2);
        },
    ...
    var Shop = Backbone.Model.extend({
        initialize: function() {
            this.set({
                ...
                salesmens:new SalesmenCollection(),
            });
            this.set({
    ...
    var PaypadWidget = db.web.OldWidget.extend({
        ...
        performPayment: function(event) {
            ...
            test = $('#net-total').text()
            if ($('#net-total').text() == ""){
            $(QWeb.render('AddToPos', {widget: self})).insertBefore('#paymentlines')
            $('#net-total').html("0.00");
            }
            $('input#discount-percentage').change(_.bind(this.setDiscountAmount, this));
            $('input#conv_amont_ele').change(_.bind(this.setDiscountAmount, this));
            $('input#discount-amount').change(_.bind(this.setDiscountPercentage, this));
        ...
        setDiscountPercentage:function(){
            ...
            currentOrder.setDiscountPercentage()
        },
        setDiscountAmount:function(){
            currentOrder = this.shop.get('selectedOrder');
    ...
    var PartnerWidget = db.web.OldWidget.extend({
        tag_name:'option',
        template_fct: qweb_template('pos-partner-template'),
        init: function(parent, options) {
            this._super(parent);
            this.model = options.model;
            this.shop = options.shop;
    ...
    var PartnerListWidget = db.web.OldWidget.extend({
    
        init: function(parent, options) {
            this._super(parent);
            this.model = options.model;
            this.shop = options.shop;
            this.shop.get('partners').bind('reset', this.render_element, this);
        },
        start: function(options) {
            $(this.$element).change(_.bind(this.changed, this));
        },
        changed: function(event){
            var partner = $("option:selected",this.$element).text();
            var partner_id = $("option:selected",this.$element).children("input").val();
            this.shop.get('selectedOrder').set({"partner": partner});
            this.shop.get('selectedOrder').set({"partner_id": partner_id});
            if (partner_id !== '0'){
                pos.set({'partner_selected': partner})
            }
            else{
                pos.set({'partner_selected': ''})
            }
            //$("#partnerid").html("");
            //if(salesman!="Select")
            $("#partnerid").html(partner);
        },
        render_element: function() {
            this.$element.empty();
            (this.shop.get('partners')).each(_.bind( function(partners) {
                var p = new PartnerWidget(null, {
                        model: partners,
                        shop: this.shop
                });
                p.appendTo(this.$element);
            }, this));
            var partner = pos.get('default_pos_partner')
            if (partner){
                var partner_name = partner[1]
                var partner_id = partner[0]
                var partner_ele = document.getElementById('partner');
                partner_ele.value = partner_name
                this.shop.get('selectedOrder').set({"partner": partner_name});
                this.shop.get('selectedOrder').set({"partner_id": partner_id});
                this.shop.get('selectedOrder').set({"partnerid": partner_name});
                pos.set({'partner_selected': partner_name})
            }
            return this;
        },
    });
    
    var SalesmanWidget = db.web.OldWidget.extend({
        tag_name:'option',
        template_fct: qweb_template('pos-salesman-template'),
        init: function(parent, options) {
            this._super(parent);
            this.model = options.model;
            this.shop = options.shop;
    ...
    var SalesmenListWidget = db.web.OldWidget.extend({
        init: function(parent, options) {
            this._super(parent);
            this.model = options.model;
            this.shop = options.shop;
            
            this.shop.get('salesmens').bind('reset', this.render_element, this);
        },
        start: function(options) {
            $(this.$element).change(_.bind(this.changed, this));
        },
        changed: function(event){
            ...
            var salesman = $("option:selected",this.$element).text();
            var salesman_id = $("option:selected",this.$element).children("input").val();
            this.shop.get('selectedOrder').set({"salesman": salesman});
            this.shop.get('selectedOrder').set({"salesman_id": salesman_id});
            if (salesman_id !== '0'){
                pos.set({'salesmen_selected': salesman})
            }
            else{
                pos.set({'salesmen_selected': ''})
            }
            $("#salesmanid").html("");
            if(salesman!="Select")
                $("#salesmanid").html(salesman);
        },
        render_element: function() {
            this.$element.empty();
            (this.shop.get('salesmens')).each(_.bind( function(salesmens) {
                var p = new SalesmanWidget(null, {
                        model: salesmens,
                        shop: this.shop
                });
                p.appendTo(this.$element);
            }, this));
            var salesmen_list = this.shop.get('salesmens')
            var login_user = this.session.uid
            var def_sales_man_id = ''
            var def_sales_man_name = ''
            salesmen_list.each(function(salesman_data){
                salesman_id = salesman_data.get('id')
                if (salesman_id === login_user){
                    def_sales_man_id = salesman_id
                    salesman_name = salesman_data.get('name')
                    def_sales_man_name = salesman_name
                    var salesman_ele = document.getElementById('salesman');
                    salesman_ele.value = salesman_name
                }
            });
            if (def_sales_man_id){
                this.shop.get('selectedOrder').set({"salesman": def_sales_man_name});
                this.shop.get('selectedOrder').set({"salesman_id": def_sales_man_id});
                pos.set({'salesmen_selected': def_sales_man_name})
            }
            return this;
        },
    });
    ...
    var ProductWidget = db.web.OldWidget.extend({
        tag_name:'li',
        template_fct: qweb_template('sfs-pos-product-template'),
        init: function(parent, options) {
            this._super(parent);
            this.model = options.model;
            this.shop = options.shop;
    ...
    var PaymentlineWidget = db.web.OldWidget.extend({
        tag_name: 'table',
        template_fct: qweb_template('sfs-pos-paymentline-template'),
        init: function(parent, options) {
            this._super(parent);
            this.model = options.model;
...
//        },
        ...
        validate:function(){
            $('button#validate-order').attr('disabled', 'disabled');
            var currentOrder, total = parseFloat(0);
            $(".pl-amount").each(function(){
                total += parseFloat($('input',$(this)).val());
            });
            paymentdue = parseFloat($("#net-total").text());
            if(total.toFixed(2) >= paymentdue){
                $('button#validate-order').removeAttr('disabled');
        ...
        changeAmount: function(event) {
            ...
            if (newAmount && !isNaN(newAmount)) {
                ...
                crr_rate = this.model.get('curr_rate')
                conv_amount = amount / crr_rate
            	this.amount = conv_amount;
                this.model.set({
                    amount: this.amount,
                });
            }
        ...
        render_element: function() {
            ...
            this.$element.html(this.template_fct({
                name: (this.model.get('journal_id'))[1],
                amount: this.amount * this.model.get('curr_rate'),
                conv_amount: this.amount * this.model.get('comp_curr_rate'),
                conv_name: "Amount in MXN ($)"
            }));
    ...
    var PaymentWidget = db.web.OldWidget.extend({
        ...
        validateCurrentOrder: function() {
            ...
            currentOrder = this.shop.get('selectedOrder');
            currentOrder.setNetTotal()
            currentOrder.setDiscountAmount()
            $('button#validate-order', this.$element).attr('disabled', 'disabled');
            pos.pushOrder(currentOrder.exportAsJSON()).then(_.bind(function() {
                disc = pos.get('disc')
                if (disc != false){
                    $("#amount_in_words").html(pos.get('amount_in_words'));
                    $("#name").html(pos.get('name'));
                    if(pos.get('company_address') != false )
	                    $("#company_address").html(pos.get('company_address')+"<br/>");
	                else
	                    $("#company_address").html("");
                    if(pos.get('vat') != false )
	                    $("#vat").html("RFC : " + pos.get('vat')+"<br/>");
	                else
	                    $("#vat").html("");
                   if(pos.get('warehouse_address') != false )
	                    $("#warehouse_address").html("Domicilio de emisión: " + pos.get('warehouse_address')+"<br/>");
	                else
	                    $("#warehouse_address").html("");
                    if(pos.get('partner_address') != false )
                        $("#partner_address").html(pos.get('partner_address')+"<br/>");
	                else
	                    $("#partner_address").html("");
                    if(pos.get('partner_selected'))
                        $("#partnerid").html(pos.get('partner_selected')+"<br/>");
                    else
                        $("#partnerid").html(""+"<br/>");
                    if(pos.get('salesmen_selected'))
                        $("#salesman_name").html(pos.get('salesmen_selected')+"<br/>");
                    else
                        $("#salesman_name").html(""+"<br/>");
        ...
        updatePaymentSummary: function(cashRegister) {
            var currentOrder, dueTotal, paidTotal, remaining, remainingAmount;
            currentOrder = this.shop.get('selectedOrder');
            paidTotal = currentOrder.getPaidTotal();
            dueTotal = currentOrder.getNetAmount();
            netTotal = currentOrder.getNetAmount();
    ...
    var ReceiptWidget = db.web.OldWidget.extend({
        init: function(parent, options) {
            ...
            this.company_address = pos.get('company_address');
            this.vat = pos.get('vat');
            this.warehouse_address = pos.get('warehouse_address');
            this.name = pos.get('name');
            this.partner_address = pos.get('partner_address');
        },
        start: function() {
            this.shop.bind('change:selectedOrder', this.changeSelectedOrder, this);
        ...
        finishOrder: function() {
            var test = document.getElementById('salesman').options;
            var salesmans = this.shop.get('salesmens')
            var partner = pos.get('default_pos_partner')
            var login_user = this.session.uid
            var def_saleman_name = 'Select'
            var def_saleman_id = 0
            var partner_name = 'Select'
            var partner_id = 0
            if (partner){
                partner_name = partner[1]
                partner_id = partner[0]
            }
            salesmans.each(function(salesman_data){
                salesman_id = salesman_data.get('id')
                if(salesman_id === login_user){
                    salesman_name = salesman_data.get('name')
                    def_saleman_name = salesman_name
                    def_saleman_id = salesman_id
                }
            });
            this.shop.get('selectedOrder').destroy();
            this.shop.get('selectedOrder').set({"partner": partner_name});
            this.shop.get('selectedOrder').set({"partner_id": partner_id});
            this.shop.get('selectedOrder').set({"partnerid": partner_name});
            this.shop.get('selectedOrder').set({"salesman": def_saleman_name});
            this.shop.get('selectedOrder').set({"salesman_id": def_saleman_id});
            var salesman_ele = document.getElementById('salesman');
            salesman_ele.value = def_saleman_name
            var partner_ele = document.getElementById('partner');
            partner_ele.value = partner_name
            pos.set({'salesmen_selected': def_saleman_name})
            pos.set({'partner_selected': partner_name})
        },
        changeSelectedOrder: function() {
        ...
        refresh: function() {
            ...
            $('.pos-receipt-container', this.$element).html(qweb_template('AddToPosTicket')({widget:this}));
        },
    });
    ...
    var ShopWidget = db.web.OldWidget.extend({
        ...
        start: function() {
            ...
            this.partnerListView = new PartnerListWidget(null, {
                shop: this.shop
            });
            this.partnerListView.$element = $("#partner");
            this.partnerListView.render_element();
            this.partnerListView.start();
            this.salesmenListView = new SalesmenListWidget(null, {
                shop: this.shop
            });
            this.salesmenListView.$element = $("#salesman");
            this.salesmenListView.render_element();
            this.salesmenListView.start();
    ...
    var App = (function() {
        ...
        App.prototype.category = function(id) {
            var c, products, partners, self = this;
            ...
            partners = pos.store.get('res.partner');
            partners.splice(0,0,{"name":"Select","id":0});
            (this.shop.get('partners')).reset(partners);
            salesmens = pos.store.get('res.users').filter( function(p) {
                var _ref;
                return _ref = p.shop_id[0], _.indexOf([pos.get('shop').id], _ref) >= 0;
            });
            salesmens.splice(0,0,{"name":"Select","id":0});
            (this.shop.get('salesmens')).reset(salesmens);
            ...
            var getProductByEAN = function (ean) {
                ...
                scannedProductModel = _.detect(allProducts, function (pc) { return pc.ean13 === ean || Number(pc.ean13) === Number(ean) || pc.code === ean || Number(pc.code) === Number(ean) });
                return scannedProductModel;
            }
           
            var searchProduct = function (s) {
                var scannedProductModel;
                scannedProductModel = allProducts.filter(function (p) {
                    return p.name.toLowerCase().indexOf(s) != -1 || p.code.indexOf(s) != -1 || (p.ean13 && p.ean13.indexOf(s) != -1);
                });
                return scannedProductModel;
            }
            var searchText = function (text) {
                var queryTerms = text.split(' ');
                scannedProductModel = allProducts;
                for (var i = 0; i < queryTerms.length; i++) {
                    var s = queryTerms[i];
                    scannedProductModel = scannedProductModel.filter(function (p) {
            ...
            $('.searchbox input').keyup(function (event) {
                ...
                if (!isNaN(Number(s))) {
                    myproduct = getProductByEAN(s);
                    if (myproduct) {
                        selectedOrder.addProduct(new Product(myproduct));
                        $(this).val('');
                    }
                    else {
                        $('.search-clear').fadeIn();
                    ...
                    if (!isNaN(Number(String.fromCharCode(e.which)))) {
                        ...
                        if (codeNumbers.length == 0) {
                            ...
                            if (lastTimeStamp + 30 < new Date().getTime()) {
                                // not a barcode reader go to search box
                                var $searchControl = $('.searchbox input');
                                codeNumbers.push(String.fromCharCode(e.which));
                                $searchControl.focus().val(codeNumbers.join(''));
                                timeStamp = new Date().getTime();
                                codeNumbers = [];
                                $('.search-clear').fadeIn();
                                return;
                        ...
                        if (e.which == 13) {
                            console.log(codeNumbers.join(''));
                            var selectedOrder = self.shop.get('selectedOrder');
                            var code = codeNumbers.join('');
                            if (code == "") {
                                return;
                            }
                            var scannedProductModel = getProductByEAN(code);
