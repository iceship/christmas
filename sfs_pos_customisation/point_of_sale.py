...
class point_of_sale(osv.osv):
    ...
    _columns = {
                ...
                'with_invoice': fields.boolean('With Invoice'),
    ...
    def create_from_ui(self, cr, uid, orders, context=None):
        ...
        for order in orders:
            ...
            if order.get('partner_id', 0) == 0 or order.get('partner_id', '0') == '0':
        ...
        res = super(point_of_sale, self).create_from_ui(cr, uid, orders, context=context)
        return res
        ...
        if not res:
            for order in self.browse(cr, uid, ids, context=context):
                if (not order.lines) or (not order.statement_ids) or \
                    (order.amount_total-(order.amount_paid + order.discount_amount)> 0.00001):
                    return False
                else:
                    return True
        return res
...
class pos_order_line(osv.osv):
    ...
    def _amount_line_all(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            taxes_id = [x for x in line.product_id.taxes_id if (x.company_id == False or x.company_id.id in company_ids)]
